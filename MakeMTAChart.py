...
TOP_LEVEL_TO_PLOT = 2 # In team gantt, 2 is the major levels (level 1 is the project level)
...
def checkIfShouldInclude(task_number, tasks_to_include, tasks_to_exclude):
...
def make_plot_for_tasks(task_objs, plot_level, tasks_to_include, tasks_to_exclude, plot_title):
    ...
    max_date = datetime(year=1, month=1, day=1)
    min_date = this_month


    # Get max and min dates for this plot 
    # Go through each task and add to plot
    for task in task_objs:
        this_level = task['level']
        this_number = task['task_number'].strip()
        ...
        if not checkIfShouldInclude(this_number, tasks_to_include, tasks_to_exclude):
            continue
        ...
        if min(these_calculated_dates) < min_date:
            min_date = min(these_calculated_dates)
    ...
    ax.set_xlabel('Date report generated')
    ax.set_ylabel('Projected completion date')

    # show the plot
    #plt.show()


# Import the tasks from the csv file into task_objs, and a list of the top level tasks
def import_tasks_from_csv(full_fname_in, tasks_to_include, tasks_to_exclude, top_level_to_plot):
    # Check for CURRENT.csv
    data_fname = full_fname_in

    if not os.path.isfile(data_fname):
        print('No file at location: ', data_fname)
    ...
    for line in reader:
        ...
        # Skip level 1 (this is just the project header...) and anything above top_level_to_plot
        if (this_level < top_level_to_plot) | (this_level < 2):
            continue
        ...
        # Check if shoudl include
        if not checkIfShouldInclude(this_task_number, tasks_to_include, tasks_to_exclude):
            continue
        ...
        # keep a list of top-level parent tasks
        if this_level == top_level_to_plot:
            top_level_tasks.append(this_obj)
    ...
    return task_objs, top_level_tasks
...
def main():
    ...
    task_objs, top_level_tasks = import_tasks_from_csv(full_fname_in, TASKS_TO_INCLUDE, TASKS_TO_EXCLUDE, TOP_LEVEL_TO_PLOT)
